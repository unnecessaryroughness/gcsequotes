...
module.exports = {
  canHandle(handlerInput) {
    ...
    return type === 'IntentRequest' && intent.name === 'AMAZON.YesIntent'
  },
  handle(handlerInput) {
    ...
    switch (sessionData.lastAction) {
      case "answerquestion": 
        ...
        sessionData.questionList[questionIndex].correct = true
        sessionData.score += 1
        sessionData = askQuestion(sessionData)
        speechText = parseSpeech('gameplay', 'congratulations')
        ...
        break
      default: 
        speechText = parseSpeech('errors', 'yes_out_of_context')
